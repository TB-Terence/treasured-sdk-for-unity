...
namespace Simple360Render
{
	public static class I360Render
	{
		...
		public static byte[] Capture(ref RenderTexture cubemap, int width = 1024, bool encodeAsJPEG = true, Camera renderCam = null, bool faceCameraDirection = true)
		{
			if (renderCam == null)
			{
				renderCam = Camera.main;
			...
			int cubemapSize = Mathf.Min(Mathf.NextPowerOfTwo(width), 8192);
			RenderTexture activeRT = RenderTexture.active;
			RenderTexture equirectangularTexture = null;
			Texture2D output = null;
			try
			{
				equirectangularTexture = RenderTexture.GetTemporary(cubemapSize, cubemapSize / 2, 0);
