...
#endif
                ImageUtilies.ConvertToKTX2(Path.Combine(TreasuredSDKPreferences.Instance.customExportFolder, Map.projectFolder, ".treasured", Map.projectFolder));
            }
        }
        ...
        private void ExportCubemap(ImageQuality imageQuality)
        {
            ...
            try
            {
                for (int index = 0; index < count; index++)
                {
                    ...
                    var path = Directory.CreateDirectory(Path.Combine(TreasuredSDKPreferences.Instance.customExportFolder, Map.projectFolder, ".treasured", Map.projectFolder, "images", exportAllQualities ? imageQuality.ToString().ToLower() : "", current.Id).ToOSSpecificPath());
                    switch (cubemapFormat)
                    {
                        case CubemapFormat._3x2:
                            // FORMAT:
